...
/**
 * Dealer Login Page
 * OTP-based authentication for Dealer users
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'send_otp') {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $auth = new Auth();
            $result = $auth->generateOTP($email, 'dealer');
            
            if ($result['success']) {
                $_SESSION['otp_email'] = $email;
                $_SESSION['otp_user_type'] = 'dealer';
        ...
        if (empty($otp)) {
            ...
            $auth = new Auth();
            $result = $auth->verifyOTP($email, 'dealer', $otp);
...
<head>
    ...
    <title>Dealer Login - Laguna Partners Portal</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/bootstrap-icons.css">
    <style>
        body {
            background: linear-gradient(135deg, #d6663c 0%, #f5a962 100%);
            min-height: 100vh;
            display: flex;
            align-items: center;
        ...
        .btn-primary:hover {
            background: linear-gradient(135deg, #f5a962 0%, #d6663c 100%);
        }
        .portal-link {
            text-align: center;
            margin-top: 20px;
            ...
            <div class="login-body">
                ...
                <?php if ($step === 'email'): ?>
                    ...
                    <div class="portal-link">
                        <small class="text-muted">Are you a vendor?</small><br>
                        <a href="<?= BASE_PATH; ?>/vendor-login.php" class="btn btn-outline-secondary btn-sm mt-2">
                            <i class="bi bi-arrow-left"></i> Vendor Login
                        </a>
                    </div>
                <?php else: ?>
                    <!-- Step 2: Enter OTP -->
                    <div class="text-center mb-4">
                        <i class="bi bi-envelope-check" style="font-size: 3rem; color: #d6663c;"></i>
                    ...
                    <form method="POST" action="">
                        ...
                        <a href="<?php echo BASE_PATH; ?>/dealer-login.php" class="btn btn-outline-secondary w-100">
