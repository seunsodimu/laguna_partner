...
/**
 * Vendor Login Page
 * OTP-based authentication for Vendor users
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'send_otp') {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $auth = new Auth();
            $result = $auth->generateOTP($email, 'vendor');
            
            if ($result['success']) {
                $_SESSION['otp_email'] = $email;
                $_SESSION['otp_user_type'] = 'vendor';
        ...
        if (empty($otp)) {
            ...
            $auth = new Auth();
            $result = $auth->verifyOTP($email, 'vendor', $otp);
...
<head>
    ...
    <title>Vendor Login - Laguna Partners Portal</title>
            ...
            <div class="login-body">
                ...
                <?php if ($step === 'email'): ?>
                    ...
                    <div class="portal-link">
                        <small class="text-muted">Are you a dealer?</small><br>
                        <a href="<?= BASE_PATH; ?>/dealer-login.php" class="btn btn-outline-secondary btn-sm mt-2">
                            <i class="bi bi-arrow-left"></i> Dealer Login
                        </a>
                    </div>
                <?php else: ?>
                    <!-- Step 2: Enter OTP -->
                    ...
                    <form method="POST" action="">
                        ...
                        <a href="<?php echo BASE_PATH; ?>/vendor-login.php" class="btn btn-outline-secondary w-100">
